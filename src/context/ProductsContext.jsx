...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch('https://68dadf3a23ebc87faa317c19.mockapi.io/api/productos');
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://68dadf3a23ebc87faa317c19.mockapi.io/api/productos/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
const eliminarProducto = async (id) => {
  try {
    const respuesta = await fetch(
      `https://68dadf3a23ebc87faa317c19.mockapi.io/api/productos/${id}`,
      {
        method: 'DELETE',
      }
    );
    ...
    throw error; // se manejará en el componente con toast
  }
};
